...
void drive_suck_msec(int speed, int suckspeed, int duration) {
  setDriveSpeed(speed);
...
/* drive_suck_linetube
* @purpose: autonomously drive robot forward and suck until tubes are collected or a
* disatance is travled in inches
* precondition: 4 inch wheels used; if other wheels, change the 4 in the while loop
* hardware: encoders, line tracker in collector for tube detection
...
* @param tubedistance: desired value of line tracker out put. Usually 3075
...
void drive_suck_linetube(int speed, int suckspeed, float distance) {
  SensorValue[EncoderL] = 0; //zeros encoders
  SensorValue[EncoderR] = 0;

  while( (tube_detection()) || // is dependent on the "bool tube_detection" function//
    ( ( (float) abs(SensorValue[EncoderL])  * 4 / 360 * PI) < distance &&
...
void drive_suck(int speed, int suckspeed, float distance) {
