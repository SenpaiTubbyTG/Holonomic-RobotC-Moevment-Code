...
public class FusorDetector implements FeatureDetector, FeatureListener {
	...
	private ArrayList detectors = null;

	private ArrayList readings = null;
	...
	public Feature scan() {
		...
		FeatureDetector d = (FeatureDetector) it.next();
		Feature df = d.scan();
		if(df != null) {
		...
		public void run() {
			while(true) {
				...
				if(enabled) {
					...
					RangeReadings r = (RangeReadings) it.next();
					    for (Iterator it2 = r.iterator(); it2.hasNext();) {
						RangeReading rtemp = (RangeReading) it2.next();
						rrs.add(rtemp);
					    }
				    }
	...
	public void enableDetection(boolean on) {
		enabled = on;
	    for (Iterator it = detectors.iterator(); it.hasNext();) {
		FeatureDetector fd = (FeatureDetector) it.next();
