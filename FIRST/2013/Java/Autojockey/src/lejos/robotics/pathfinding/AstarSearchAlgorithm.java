...
public class AstarSearchAlgorithm implements SearchAlgorithm{
	...
	public Path findPath(Node start, Node goal) {
		...
		ArrayList  closedset = new ArrayList(); // The set of nodes already evaluated. Empty at start.
		ArrayList  openset = new ArrayList(); // The set of tentative nodes to be evaluated.
		...
		while (!openset.isEmpty()) {
			...
			Collection  yColl = x.getNeighbors();
			Iterator  yIter = yColl.iterator();

			while(yIter.hasNext()) { // for each y in neighbor_nodes(x)
				neighbor_loop++; // TODO: DELETE ME
				Node y = (Node) yIter.next();
	 ...
	 */
	private static Node getLowestCost(Collection nodeSet) {
		...
		Node best = (Node) nodeIterator.next();
		while(nodeIterator.hasNext()) {
			Node cur = (Node) nodeIterator.next();
	 ...
	 */
	private static final void reconstructPath(Node current_node, Node start, Collection path){
