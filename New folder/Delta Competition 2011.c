...
task autonomous()
{
  ...
  while(hitwall == false) //runs fowards until the wall is hit
  {

 if(SensorValue(RightBumper) == 1)
     {//if right side hits wall, stop right motor
        motor[port2] = 0;
     }
      else if(SensorValue(LeftBumper) == 1)
     {//if left side hits wall, stop left motor
        motor[port3] = 0;
     }
     if (SensorValue(RightBumper) == 1 && SensorValue(LeftBumper) == 1)
     {
       motor[port2] = 0;
       motor[port3] = 0;
       hitwall = true;      //:when both hit, the loot is finished and driving motors power off
     }
  }
