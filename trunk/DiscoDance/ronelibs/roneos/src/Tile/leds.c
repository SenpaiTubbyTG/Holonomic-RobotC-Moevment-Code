...
void led_latch(Tile tile);
void led_latch_onoff_data(Tile tile, uint32 code);
void led_latch_dimmer_data(Tile tile);
 ...
 */
void led_latch(Tile tile) {
	MAP_GPIOPinWrite(tile.LED_SEL_PORT, tile.LED_SEL_PORT, tile.LED_SEL_PIN);
	MAP_SysCtlDelay(1);
	MAP_GPIOPinWrite(tile.LED_SEL_PORT, tile.LED_SEL_PIN, 0);
}
...
void led_latch_dimmer_data(Tile tile) {
	...
	for (i = 0; i < LED_NUM_ELEMENTS; i++) {
		if (led_dimmers_old[i] != tile.led_dimmers[i]) {
			led_dimmers_old[i] = tile.led_dimmers[i];
			update_data = true;
		}
	}
	...
	if (update_data) {
		i = 0;
		led_dimmers_shifted[i++] = tile.led_dimmers[15];
		led_dimmers_shifted[i++] = tile.led_dimmers[LED_RED_START_IDX + 4];
		led_dimmers_shifted[i++] = tile.led_dimmers[LED_RED_START_IDX + 3];
		led_dimmers_shifted[i++] = tile.led_dimmers[LED_RED_START_IDX + 2];
		led_dimmers_shifted[i++] = tile.led_dimmers[LED_RED_START_IDX + 1];
		led_dimmers_shifted[i++] = tile.led_dimmers[LED_RED_START_IDX + 0];

		led_dimmers_shifted[i++] = tile.led_dimmers[LED_GREEN_START_IDX + 4];
		led_dimmers_shifted[i++] = tile.led_dimmers[LED_GREEN_START_IDX + 3];
		led_dimmers_shifted[i++] = tile.led_dimmers[LED_GREEN_START_IDX + 2];
		led_dimmers_shifted[i++] = tile.led_dimmers[LED_GREEN_START_IDX + 1];
		led_dimmers_shifted[i++] = tile.led_dimmers[LED_GREEN_START_IDX + 0];

		led_dimmers_shifted[i++] = tile.led_dimmers[LED_BLUE_START_IDX + 4];
		led_dimmers_shifted[i++] = tile.led_dimmers[LED_BLUE_START_IDX + 3];
		led_dimmers_shifted[i++] = tile.led_dimmers[LED_BLUE_START_IDX + 2];
		led_dimmers_shifted[i++] = tile.led_dimmers[LED_BLUE_START_IDX + 1];
		led_dimmers_shifted[i++] = tile.led_dimmers[LED_BLUE_START_IDX + 0];
...
//		tiles[tileLoopCtr].led_dimmers = { 0 };
		led_set_dimmer_all(tiles[tileLoopCtr], 0);
		led_latch_dimmer_data(tiles[tileLoopCtr]);
		led_latch_onoff_data(tiles[tileLoopCtr], LED_ONOFF_ALL_MASK);
		leds_set(tiles[tileLoopCtr], LED_RED, LED_PATTERN_CIRCLE, LED_BRIGHTNESS_HIGH, LED_RATE_FAST);
	}
}
 ...
 * @returns void
 */
void led_set_dimmer(Tile tile, uint32 led_idx, uint32 val) {
...
void led_set_dimmer_color(Tile tile, uint32 led_color, uint32 dimmer) {
	...
	for (i = startIdx; i < (startIdx + LED_NUM_ELEMENTS_PER_COLOR); i++) {
		...
		//to detect errors!!
		led_set_dimmer(tile, i, (uint8) dimmer);
...
void led_set_dimmer_all(Tile tile, uint32 dimmer) {
	...
	int i;
	for (i = 0; i < LED_NUM_ELEMENTS; i++) {
		tile.led_dimmers[i] = (uint8) dimmer;
	}
}
...
void leds_update(Tile tile) {
	...
	if (led_animation_pattern == LED_PATTERN_MANUAL) {
		...
		led_set_dimmer_all(tile, 0);
		switch (led_animation_pattern) {
		case LED_PATTERN_ON: {
			for (c = colorStartIdx; c <= colorEndIdx; c++) {
				led_set_dimmer_color(tile, c, led_animation_brightness);
			}
			break;
		}
		...
		case LED_PATTERN_PULSE: {
			...
			for (c = colorStartIdx; c <= colorEndIdx; c++) {
				if (led_animation_pattern == LED_PATTERN_BLINK) {
					led_set_dimmer_color(tile, c, (blink_pattern[pattern_counter]
					...
					led_set_dimmer_color(tile, c, (val * led_animation_brightness)
							>> LED_PATTERN_SHIFTS);
				}
			}
			break;
		...
		case LED_PATTERN_CIRCLE: {
			...
			for (c = colorStartIdx; c <= colorEndIdx; c++) {
				...
				for (i = 0; i < LED_NUM_ELEMENTS_PER_COLOR; ++i) {
					led_set_dimmer(tile, idx++, ((uint32) circle_state[i]
