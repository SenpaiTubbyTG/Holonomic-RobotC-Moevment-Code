...
void SPIInit(void) {
	...
	MAP_SysCtlPeripheralEnable(MSP430_SELECT_SYSCTL);
	...
	MAP_GPIOPinTypeGPIOOutput(MSP430_SELECT_PORT, MSP430_SELECT_PIN);
...
void SPIDeselectISR() {
	...
	MAP_GPIOPinWrite(MSP430_SELECT_PORT, MSP430_SELECT_PIN, MSP430_SELECT_PIN);
	MAP_GPIOPinWrite(SDCARD_SELECT_PORT, SDCARD_SELECT_PIN, SDCARD_SELECT_PIN);
...
void SPISelectDeviceISR(uint8 device) {
	...
	switch (device) {
	case SPI_RADIO: {
		SPIConfigure(SSI0_BASE, SPI_RADIO_WORDSIZE, SSI_FRF_MOTO_MODE_0);
		MAP_GPIOPinWrite(RADIO_SELECT_PORT, RADIO_SELECT_PIN, 0);
		break;
	}
	case SPI_ACCELEROMETER: {
		//SPIConfigure(SSI0_BASE, SPI_ACCELEROMETER_WORDSIZE, SSI_FRF_MOTO_MODE_0);
	...
	case SPI_MSP430: {
		SPIConfigure(SSI0_BASE, SPI_MSP430_WORDSIZE, SSI_FRF_MOTO_MODE_1);
		MAP_GPIOPinWrite(MSP430_SELECT_PORT, MSP430_SELECT_PIN, 0);
		break;
	}
	case SPI_SDCARD: {
		SPIConfigure(SSI0_BASE, SPI_SDCARD_WORDSIZE, SSI_FRF_MOTO_MODE_0);
		MAP_GPIOPinWrite(SDCARD_SELECT_PORT, SDCARD_SELECT_PIN, 0);
	}
	case SPI_NULL:
	default: {
		break;
