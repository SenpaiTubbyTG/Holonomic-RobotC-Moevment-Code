...
/*
 * This class is copied from the WPI PIDController because they made the calculate method,
 * the only one anyone would want to change, private. 
 * 
 * So we had to move the whole thing just to make some simple changes to the integral suggested by team 57. 
 */
package disco.utils;
 ...
 * PIDOutput
 */
public class DiscoPIDController implements IUtility, LiveWindowSendable{
    ...
    private DiscoPIDController.Tolerance m_tolerance;	//the tolerance object used to check if on target
    ...
    public class PercentageTolerance implements DiscoPIDController.Tolerance {
	double percentage;
    ...
    public class AbsoluteTolerance implements DiscoPIDController.Tolerance {
	double value;
    ...
    public class NullTolerance implements DiscoPIDController.Tolerance {
    ...
    private class PIDTask extends TimerTask {
        ...
        private DiscoPIDController m_controller;

        public PIDTask(DiscoPIDController controller) {
            if (controller == null) {
     ...
     */
    public DiscoPIDController(double Kp, double Ki, double Kd, double Kf,
        ...
        m_controlLoop.schedule(new DiscoPIDController.PIDTask(this), 0L, (long) (m_period * 1000));

        instances++;
        UsageReporting.report(UsageReporting.kResourceType_PIDController, instances);
	m_tolerance = new DiscoPIDController.NullTolerance();
    }
     ...
     * @param period 
     */
    public DiscoPIDController(double Kp, double Ki, double Kd,
            PIDSource source, PIDOutput output,
            double period) {
...
//                }
    ...
    /**
     * Set the setpoint for the DiscoPIDController
    ...
    /**
     * Returns the current setpoint of the DiscoPIDController
     ...
     * creating the type of tolerance that you want to use: setTolerance(new DiscoPIDController.AbsoluteTolerance(0.1))
     * @param tolerance a tolerance object of the right type, e.g. PercentTolerance
     * or AbsoluteTolerance
     */
    private void setTolerance(DiscoPIDController.Tolerance tolerance) {
	...
	m_tolerance = new DiscoPIDController.AbsoluteTolerance(absvalue);
    }
	...
	m_tolerance = new DiscoPIDController.PercentageTolerance(percentage);
    }
    ...
    /**
     * Begin running the DiscoPIDController
     */
    ...
    /**
     * Stop running the DiscoPIDController, this sets the output to zero before stopping.
     */
    ...
    /**
     * Return true if DiscoPIDController is enabled.
     */
